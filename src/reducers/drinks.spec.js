...
describe('drinks reducer', () => {
  ...
  it('DRINK_CREATE: should return the created drink', () => {
    const state = reducer([], {type: 'DRINK_CREATE', payload: {name: 'New Drink'}});
  ...
  it('DRINK_RECORD_IST: should set the ist value', () => {
    ...
    const state = reducer([initialState], {type: 'DRINK_RECORD_IST', payload: {id: 'drink-1', ist: 23}});
    expect(state[0].ist).to.equal(23)
  })

  it('DRINK_UPDATE: should update the a drink', () => {
    const state = reducer(initialState, {type: 'DRINK_UPDATE', payload: {id: 'drink-1', max: 42}});
